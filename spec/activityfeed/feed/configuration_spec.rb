  ...
  module Feed
    describe Configuration do
      ...
      context 'creating feed configurations' do
        ...
        it('responds to :feed') { is_expected.to respond_to(:feed) }
        it('responds to :feed!') { is_expected.to respond_to(:feed) }
        ...
        context 'when defining a specific feed' do
          ...
          before do
            multi_config.feed(feed_name) do |config|
              config.backend = :new_backend
            end
          end

          subject { ActivityFeed.feed(feed_name) }
          ...
          context 'when passing namespace as argument' do
            subject { ActivityFeed.feed(:follower_activity, :ar).namespace }
        ...
        context 'various ways of accessing configuration' do
          let(:backend) { :mongodb }
          before :each do
            ActivityFeed.feed(:my_feed) { |c| c.backend = backend; c.namespace = :mfd; c.per_page = 60 }
          end
      ...
      context 'generating feed instances' do
        ...
        context 'for multiple users' do
          ...
          context 'a user array' do
            let(:sample_feed) { ActivityFeed.feed(:sample_feed).for(user_list) }
          ...
          context 'for a user proc' do
            ...
            let(:sample_feed) { ActivityFeed.feed(:sample_feed).for(user_proc) }
            it('should return Collection') { expect(sample_feed).to be_kind_of(ActivityFeed::User::Collection) }
        ...
        context 'for a single user' do
          let(:sample_feed) { ActivityFeed.feed(:sample_feed).for(user1) }
